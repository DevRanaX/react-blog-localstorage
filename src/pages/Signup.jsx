...
export default function Signup() {
  ...
  const [captcha, setCaptcha] = useState(generateChapture(6));
  const signupHandler = (e) => {
    e.preventDefault();
    const { username, email, password, c_pass, chapture, cp, profile } =
      e.target;
    ...
    if (chapture.value.trim().toLowerCase() !== captcha.trim().toLowerCase()) {
              ...
              <div className="flex flex-col mb-2">
                ...
                <div className="py-2 font-elite text-xl font-bold">
                  <input
                    ...
                    name="cp"
                    value={captcha}
                    className="text-center border border-gray-300 px-2 py-1 rounded focus:outline-none border-none "
                    readOnly
                  />
                </div>
                <input
                  type="text"
                  name="chapture"
                  placeholder="Enter above chapture"
                  className="placeholder:capitalize border border-gray-300 px-2 py-1 rounded"
                  required
                  aria-required
                />
              </div>
